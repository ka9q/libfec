...
int update_viterbi29_blk_av(void *p,unsigned char *syms,int nbits){
  ...
  while(nbits--){
    ...
    for(i=0;i<8;i+=2){
      ...
      /* Form branch metrics */
      metric = vec_avg(vec_xor(Branchtab29[0].v[i+1],sym1v),vec_xor(Branchtab29[1].v[i+1],sym2v));
      metric = vec_sr(metric,(vector unsigned char)(3));
      m_metric = (vector unsigned char)(31) - metric;
    
      /* Add branch metrics to path metrics */
      m0 = vec_adds(vp->old_metrics->v[i+1],metric);
      m3 = vec_adds(vp->old_metrics->v[9+i],metric);
      m1 = vec_adds(vp->old_metrics->v[9+i],m_metric);
      m2 = vec_adds(vp->old_metrics->v[i+1],m_metric);
      ...
      /* Interleave and store decisions and survivors */
      d->v[2*i+2] = vec_mergeh(decision0,decision1);
      d->v[2*i+3] = vec_mergel(decision0,decision1);
      vp->new_metrics->v[2*i+2] = vec_mergeh(survivor0,survivor1);
      vp->new_metrics->v[2*i+3] = vec_mergel(survivor0,survivor1);
    }
    d++;
