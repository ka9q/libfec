...
int update_viterbi27_blk_av(void *p,unsigned char *syms,int nbits){
  ...
  while(nbits--){
    vector unsigned char survivor0,survivor1,sym1v,sym2v;
    vector bool char decision0,decision1;
    vector unsigned char metric,m_metric,m0,m1,m2,m3;
    void *tmp;

    /* sym1v.0 = syms[0]; sym1v.1 = syms[1] */
    sym1v = vec_perm(vec_ld(0,syms),vec_ld(1,syms),vec_lvsl(0,syms));
    ...
    metric = vec_avg(vec_xor(Branchtab27[0].v[0],sym1v),vec_xor(Branchtab27[1].v[0],sym2v));
    ...
    metric = vec_avg(vec_xor(Branchtab27[0].v[1],sym1v),vec_xor(Branchtab27[1].v[1],sym2v));
    metric = vec_sr(metric,(vector unsigned char)(3));
    m_metric = vec_sub((vector unsigned char)(31),metric);
    ...
    for(i=0;i<64;i++)
      fprintf(stderr," %d",vp->new_metrics->c[i]);
    fprintf(stderr,"\n");
    fprintf(stderr,"decisions: ");
    for(i=0;i<64;i++)
      fprintf(stderr,"%d",d->c[i]&1);
    fprintf(stderr,"\n");
    }
#endif
    d++;
